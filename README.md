...
8. `PromptSet JSON取込` で結果を戻す。紐づいた依頼は `プロンプト生成済み` になり、`プロンプト` タブにタイトル別のプロンプトが追加される。
9. プロンプト、絵柄、状態、個別Upscalerを確認して `編集内容を保存` を押す。
10. `全選択` またはCtrl／Shift選択で生成対象を選び、`選択項目をバッチへ` を押す。確認済みをまとめて管理する場合は `選択を生成準備済み` にしてから `生成準備済み全件をバッチへ` を押す。
...
既存プロンプトから始める場合は、`txt取込` または `PromptSet JSON取込` を使います。依頼Inboxを経由せず、そのまま `プロンプト` タブで確認してバッチへ渡せます。
...
生成側が `PromptSet JSON` を出力した場合は、SQLite管理画面の `PromptSet JSON取込` から読み込めます。現在のtxtも引き続き取り込めるため、生成側の移行を待つ必要はありません。
